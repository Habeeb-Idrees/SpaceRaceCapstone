...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig2 = px.pie(filtered_df, values=filtered_df.values, 
        names=filtered_df.index,
        title='Sucessfull launches for Site: {}'.format(entered_site))
        return fig2
# TASK 4:
...
def get_payload(entered_site, payload):
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(payload[0], payload[1])]
    if entered_site == 'ALL':
        fig1 = px.scatter(spacex_df, 
        x='Payload Mass (kg)', 
        y='class', 
        color='Booster Version Category',
        title='Corrilation between Payload and Success for all sites')
        return fig1
    else:
